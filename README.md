...
* **엔티티 설계의 중요성:** 엔티티 변수명의 일관성이 타임리프(`ModelAttribute`)와 컨트롤러 간의 데이터 전달 효율을 결정한다는 것을 체감했습니다.<br>
* **고유 식별자(ID) 중심 설계:** 닉네임과 같이 변경 가능하고 중복될 수 있는 속성이 아닌, 시스템 고유 식별자를 기준으로 모든 로직(CRUD)을 설계해야 보안 취약점을 방지할 수 있음을 배웠습니다.<br>
